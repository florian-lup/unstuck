...
import require$$1$3, { ipcMain, app, BrowserWindow, shell, globalShortcut, screen, Menu, Tray, nativeImage } from "electron";
...
class AppLifecycleManager {
   ...
   */
  registerManagers(authIPCHandlers2, shortcutsManager2, autoLaunchManager2, autoUpdaterManager2) {
    this.authIPCHandlers = authIPCHandlers2;
    this.shortcutsManager = shortcutsManager2;
    this.autoLaunchManager = autoLaunchManager2;
    this.autoUpdaterManager = autoUpdaterManager2;
  }
  /**
...
class SecureStorage {
  secureDir = require$$1.join(require$$2.homedir(), ".unstuck-secure");
  /**
  ...
  async getItem(key) {
    try {
      ...
      const filePath = require$$1.join(this.secureDir, `${key}.dat`);
      const encrypted = await fs$1.readFile(filePath);
  ...
  async setItem(key, value) {
    ...
    const filePath = require$$1.join(this.secureDir, `${key}.dat`);
    await fs$1.writeFile(filePath, encrypted, { mode: 384 });
  }
  /**
  ...
  async ensureSecureDir() {
    try {
      await fs$1.mkdir(this.secureDir, { recursive: true, mode: 448 });
    } catch {
    }
  }
}
class TokenManager {
  ...
  async refreshTokens(currentTokens) {
    ...
    try {
      ...
      throw new Error(
        `Token refresh network error: ${error2 instanceof Error ? error2.message : "Unknown error"}`
      );
    }
    if (!response.ok) {
      this.recordRefreshAttempt(refreshKey);
...
class Auth0Service {
  ...
  async initialize(domain, clientId, config) {
    ...
    this.deviceFlowManager.setEventCallback((event, tokens, error2) => {
      if (event === "SUCCESS" && tokens) {
        void this.handleDeviceFlowSuccess(tokens);
      } else if (event === "ERROR") {
        this.notifyListeners("ERROR", null, error2);
      }
    });
    ...
    if (this.currentSession) {
      if (this.tokenManager.isTokenExpired(this.currentSession.tokens)) {
        try {
          ...
          if (error2 instanceof Error && (error2.message.includes("re-authentication required") || error2.message.includes("expired too long ago") || error2.message.includes("Too many token refresh attempts"))) {
            await this.signOut();
   ...
   */
  notifyListeners(event, session, error2) {
    this.listeners.forEach((listener) => {
      try {
        listener(event, session, error2);
      } catch {
      }
    });
  }
...
class AuthIPCHandlers {
  ...
  setupAuthStateListeners() {
    auth0Service.onAuthStateChange((event, session, error2) => {
...
class AutoLaunchManager {
  ...
  constructor(appName = "Unstuck") {
    ...
    this.settingsPath = require$$1.join(
  ...
  async initializeAutoLaunch() {
    try {
      const isFirstRun = !fs$2.existsSync(this.settingsPath);
  ...
  async loadAutoLaunchSetting() {
    try {
      if (!fs$2.existsSync(this.settingsPath)) {
        return true;
      }
      const data = await fs$2.promises.readFile(this.settingsPath, "utf-8");
...
class Logger {
  ...
  info(...args) {
    if (this.isDevelopment) {
      console.info("[INFO]", (/* @__PURE__ */ new Date()).toISOString(), ...args);
    }
  }
  /**
  ...
  security(message, data) {
    const sanitizedData = data ? this.sanitizeForLogging(data) : void 0;
    if (this.isDevelopment) {
      console.info(
        "[SECURITY]",
      ...
      console.info("[SECURITY]", (/* @__PURE__ */ new Date()).toISOString(), message);
    }
  }
  /**
...
class AutoUpdaterManager {
  ...
  setupEventHandlers() {
    ...
    mainExports.autoUpdater.on("update-available", (info) => {
      ...
      mainExports.autoUpdater.downloadUpdate().catch((error2) => {
        logger.error("Error downloading update:", error2);
      });
    });
  ...
  async checkForUpdates() {
    ...
    try {
      ...
      logger.error("Failed to check for updates:", error2);
    }
  }
  /**
  ...
  sendStatusToWindow(channel, data) {
    ...
    windows.forEach((window2) => {
      if (!window2.isDestroyed()) {
        window2.webContents.send(`updater:${channel}`, data);
      }
    });
  }
  /**
