...
class AuthService {
  ...
  async secureGetItem(key) {
    try {
      ...
      return safeStorage2.decryptString(encrypted);
    } catch (error) {
      return null;
    }
  }
  async secureSetItem(key, value) {
    try {
      ...
      const encrypted = safeStorage2.encryptString(value);
      const filePath = path.join(this.secureDir, `${key}.dat`);
...
void app.whenReady().then(async () => {
  ...
  ipcMain.handle("auth-get-oauth-url", async (_event, provider) => {
    try {
      SecurityValidator.checkRateLimit("auth-get-oauth-url", 5, 6e4);
  ...
  ipcMain.handle("auth-get-session", async () => {
    try {
      SecurityValidator.checkRateLimit("auth-get-session", 10, 6e4);
  ...
  ipcMain.handle("auth-sign-out", async () => {
    try {
      SecurityValidator.checkRateLimit("auth-sign-out", 3, 6e4);
  ...
  ipcMain.handle("auth-is-secure-storage", () => {
    try {
      SecurityValidator.checkRateLimit("auth-is-secure-storage", 10, 6e4);
