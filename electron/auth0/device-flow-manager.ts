...
export class DeviceFlowManager {
  ...
  private eventCallback: ((event: DeviceFlowEvent, data?: Auth0Tokens, error?: string) => void) | null = null

  constructor(config: Auth0Config, domain: string, clientId: string, audience?: string, scope?: string) {
    this.config = config
    this.domain = domain
   ...
   */
  setEventCallback(callback: (event: DeviceFlowEvent, data?: Auth0Tokens, error?: string) => void): void {
  ...
  private async pollForDeviceAuthorization(deviceCode: string, interval: number): Promise<void> {
    ...
    this.currentPollInterval = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Device code expired
          this.cancelDeviceAuthorization()
          this.eventCallback?.('ERROR', undefined, 'Device code expired. Please try again.')
          ...
          // Other error
          this.cancelDeviceAuthorization()
          this.eventCallback?.('ERROR', undefined, data.error_description || 'Authorization failed')
        }
      } catch (error) {
    ...
    this.currentPollTimeout = setTimeout(() => {
      this.cancelDeviceAuthorization()
      this.eventCallback?.('ERROR', undefined, 'Authorization timeout. Please try again.')
